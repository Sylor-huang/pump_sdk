...
class Fun {
    ...
    getBondingCurvePDA(token) {
        return web3_js_1.PublicKey.findProgramAddressSync([Buffer.from(constant_1.BONDING_CURVE_SEED), token.toBuffer()], this.program.programId)[0];
    }
    getMetadataPDA(mpl, token) {
        const [metadataPDA] = web3_js_1.PublicKey.findProgramAddressSync([
            Buffer.from(constant_1.METADATA_SEED),
            mpl.toBuffer(),
            token.toBuffer()
        ], mpl);
        return metadataPDA;
    ...
    async getPumpfunGlobal() {
        const [globalAccPDA] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from(constant_1.GLOBAL_ACCOUNT_SEED)], new web3_js_1.PublicKey(constant_1.PUMPFUN_PROGRAM_ID));
    ...
    async compileTradeInstruction(params, method) {
        ...
        if (method === "BUY") {
            return await this.program.methods
                .buy(new anchor_1.BN(Number(params.amount)), new anchor_1.BN(Number(params.slippageCut)))
                .accounts({
            ...
            return await this.program.methods
                .sell(new anchor_1.BN(Number(params.amount)), new anchor_1.BN(Number(params.slippageCut)))
                .accounts({
                feeRecipient: globalAcc.feeRecipient,
                mint: params.token,
                associatedBondingCurve: ABC,
